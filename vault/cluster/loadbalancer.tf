...
resource "google_compute_ssl_certificate" "default" {
  name        = "vault-certs"
  project     = var.project_id
  private_key = file("${path.module}/ssl-certs-prod/${var.vault_tls_key}")
  certificate = file("${path.module}/ssl-certs-prod/${var.vault_tls_cert}")
}

#create the url map to map paths to backends
resource "google_compute_url_map" "default" {
  name        = "vault-backend-url-map"
  project     = var.project_id
...
resource "google_compute_http_health_check" "default" {
  ...
  project      = var.project_id
  request_path = var.request_path_lb
